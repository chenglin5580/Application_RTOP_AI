        ...
        if scope == self.para.GLOBAL_NET_SCOPE:  # get global network
            with tf.variable_scope(scope):
                ...
                with tf.name_scope('wrap_a_out'):
                    mu, sigma = mu, sigma + 1e-4  # 归一化反映射，防止方差为零
            with tf.name_scope('choose_a'):  # use local params to choose action
                self.B = tf.clip_by_value(mu, -1, 1)  # 根据actor给出的分布，选取
                self.C = sigma
    ...
    def work(self):
        ...
        while self.para.GLOBAL_EP < self.para.MAX_GLOBAL_EP:
            ...
            for ep_t in range(self.para.MAX_EP_STEP):  # MAX_EP_STEP每个片段的最大个数
                ...
                mu_best, sigma_best = self.AC.choose_best(s)  # 选取动作
                ...
                ep_a.append(a)
                ep_mu_best.append(mu_best)
                ep_sigma_best.append(sigma_best)
                ...
                if ep_t == self.para.MAX_EP_STEP-1 or done:  # 每个片段结束，输出一下结果
                    if self.name == 'W_0':
                        ...
                        print('----mu-----')
                        print(np.array(ep_mu_best[::50]))
                        print('----sigma-----')
                        print(np.array(ep_sigma_best[::50]))
